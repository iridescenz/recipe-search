...
export const NutritionFacts = () => {
  ...
  const searchNutrition = (e) => {
    ...
    const res = axios
      .get(
        `https://api.edamam.com/api/nutrition-data?app_id=${apiId}&app_key=${apiKey}&ingr=${
          product.match('oz', 'gr', 'slice', 'kg')
            ? encodeURIComponent(product)
            : encodeURIComponent(product + '%100 gram')
        }`
      )
        ...
        return (
          <li key={i}>
            ...
            <LinearProgressWithLabel
              value={nutrientValue}
              className={
                nutrientValue > 100 && `MuiLinearProgress-barColorPrimary`
              }
            />
          </li>
        )
  ...
  return (
    <div className='main-component'>
      ...
      <div
        className='container'
        style={{ backgroundImage: `url('nutrition.jpg')` }}
      >
        <Form
        ...
        {totalWeight === 0 && (
          <div className='noresult'>
            {' '}
            <div>{`No results found for ${productName}`}</div>{' '}
          </div>
        )}
